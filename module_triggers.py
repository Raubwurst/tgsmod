...
# TGS scripted diplomacy - now the bulk of the code has been moved to module_scripts
  (24, 0, 0,
               [(store_current_hours, ":number_game_hours_passed"),
                (troop_get_slot, ":timeline_status", "trp_player", slot_troop_timeline_current_state),
                (this_or_next|neq, ":timeline_status", TGS_END_GAME),
                (le, ":number_game_hours_passed", 24*170-6), ## DEBUG: Will need to remove this portion at some later date :DEBUG ##
                ],
   [
        (call_script, "script_tgs_update_scripted_diplomacy"),
    ]),

# TGS Timeline Event Checker
  (1, 0, 0,[
                (eq, "$g_tutorial_complete", 1),
                (neg|troop_slot_eq, "trp_player", slot_troop_timeline_current_state, TGS_END_GAME), # may need to double check this operation
            ],
   [
        (call_script, "script_tgs_timeline_duration_countdown_checker", 0), # script param = 0 to not force event to occur
    ]),
