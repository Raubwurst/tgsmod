  ...
  (0,
   ...
   ## Pre-Battle Orders & Deployment Begin
    (try_begin),    
	 ...
	 (assign, ":prefs_1_set", 0),
	 (assign, ":prefs_2_set", 0),
     (try_begin),
	    (party_slot_ge, 0, 76, 1), #The old slot_party_battle_pref_index
		(party_get_slot, ":caba_prefs_index", 0, 76), 
		...
		(try_begin), #For Horse-Archer Values
			(eq, ":pref_three", 2), 
			(assign, ":pref_three", 0),
		(try_end),		
		(party_set_slot, "p_main_party", slot_party_pref_wu_lance, ":pref_two"),
		(party_set_slot, "p_main_party", slot_party_pref_wu_harcher, ":pref_three"),
		(party_set_slot, "p_main_party", slot_party_pref_wu_spear, ":pref_four"),
		(party_set_slot, "p_main_party", slot_party_pref_dmg_tweaks, ":pref_one"),
	    (assign, ":prefs_1_set", 1),
	 (try_end),
	 (try_begin),		
		(party_slot_ge, 1, 76, 1),
		(party_get_slot, ":caba_prefs_index", 1, 76), 
		(store_div, ":pref_one", ":caba_prefs_index", 1000),
		(store_mul, ":pref_two", ":pref_one", 1000),
		(val_sub, ":caba_prefs_index", ":pref_two"),
		(store_div, ":pref_two", ":caba_prefs_index", 100),
		...
		(store_mul, ":pref_four", ":pref_three", 10),
		(store_sub, ":pref_four", ":caba_prefs_index", ":pref_four"),	
		(party_set_slot, "p_main_party", slot_party_pref_bc_continue, ":pref_two"),
		(party_set_slot, "p_main_party", slot_party_pref_div_dehorse, ":pref_four"),
		(party_set_slot, "p_main_party", slot_party_pref_bodyguard, ":pref_one"),
	    (assign, ":prefs_2_set", 1),
	 (try_end),
	 (try_begin),
	    (eq, ":prefs_1_set", 0),
		(eq, ":prefs_2_set", 0),
	 ...
	 (try_begin),
	    (neg|party_slot_ge, "p_main_party", slot_party_pbod_mod_version, 85),
     ...
     [
      ...
      #  (assign, ":troop_no", "$g_lord_long_term_count"),
        ...
        (assign, ":troop_no", "$g_lord_long_term_count"),
		   (try_begin),
			   (eq, ":iteration", 1),
				(val_sub, ":troop_no", "trp_kingdom_heroes_including_player_begin"),
			...
			(lt, ":troop_no", heroes_end),
			#A second check: don't do anything for non-promoted kingdom ladies
          ...
          (try_begin),
			...
			(try_begin),
              ...
              (else_try),
				...
				(this_or_next|eq, ":troop_reputation", lrep_ambitious),
				##diplomacy end+
        ...
        (try_begin),
			...
			##diplomacy start+
			##No auto-defection if there is no leader.
			(gt, ":faction_leader", -1),
			##To avoid certain odd cases, disable auto-indictment/defection for the
			##faction leader's spouse.  This can be handled separately elsewhere if
			##needed.
			(call_script, "script_dplmc_get_troop_standing_in_faction", ":troop_no", ":faction_leader"),
		...
		(try_begin),
			...
			(lt, ":troop_original_faction", 1),
			(assign, ":troop_original_faction", -2),
		(try_end),
		...
		(try_for_range, ":active_npc", heroes_begin, heroes_end),
			(this_or_next|is_between, ":active_npc", active_npcs_begin, active_npcs_end),
				(troop_slot_eq, ":active_npc", slot_troop_occupation, slto_kingdom_hero),
			(neq, ":active_npc", ":troop_no"),#Also add this check
			...
			(this_or_next|neq, ":troop_no", "trp_player"),
				(neg|troop_slot_eq, ":active_npc", slot_troop_met, 0),
		##diplomacy end+
			...
			##diplomacy start+  Accelerate forgiveness in exile (with original faction only)
			(try_begin),
				(this_or_next|troop_slot_ge, ":troop_no", slot_troop_occupation, dplmc_slto_exile),
					(troop_slot_ge, ":active_npc", slot_troop_occupation, dplmc_slto_exile),
				(troop_slot_eq, ":active_npc", slot_troop_original_faction, ":troop_original_faction"),
				(val_mul, ":chance_of_convergence", 2),
           	(try_end),
			##diplomacy end+
		...
		##diplomacy start+
		#Close loop (added to add support for kingdom ladies and other heroes)
		(try_end),
		(assign, reg0, ":save_reg0"),
		##diplomacy end+
        ]),
...
#   (assign, ":save_reg1", reg1),
   ...
   (try_begin),
	...
	(else_try),
		...
		(val_mul, reg1, 128),
		(val_add, reg1, DPLMC_VERSION_LOW_8_BITS),
  ...
  (try_for_parties, ":party_no"),
    ...
    (try_begin),
      ...
      (try_begin),
		...
		(val_mul, ":gift_value", 3),#The gift uses three copies of the item
		...
		(store_add, ":max_boost", ":gift_value", 50),
		(val_div, ":max_boost", 100),
		(val_add, ":gift_value", ":random"),
		(val_div, ":max_boost", 1000),
		(val_max, ":max_boost", ":relation_boost"),
		(try_begin),
			(gt, ":target_troop", 0),
			(troop_slot_eq, ":target_troop", slot_troop_occupation, slto_kingdom_hero),
			(store_troop_gold, reg1, ":target_troop"),
			(lt, reg1, 6000),
			(val_max, reg1, 0),
			(val_add, reg1, 6000),
			(val_div, reg1, 2),
			(val_mul, ":gift_value", 10000),#up to a 40% increase
			(val_div, ":gift_value", reg1),
		(else_try),
			(lt, ":target_troop", 1),
			(party_get_slot, reg1, ":target_party", slot_town_prosperity),
			(lt, reg1, 50),
			(val_max, reg1, 0),
			(val_add, reg1, 75),
			(val_mul, ":gift_value", 125),#up to a 40% increase
			(val_div, ":gift_value", reg1),
		(try_end),
...
####################################################################
  ...
  (24*2,
   ...
   (try_for_range, ":center_no", centers_begin, centers_end),
        ...
        # use center prosperity to determine additional volunteer values (intial)
        (party_get_slot, ":prosperity", ":center_no", slot_town_prosperity),
        ...
        (party_slot_eq, ":center_no", slot_party_type, spt_town),
            ...
            # Update outlander volunteers available
            (party_get_slot, ":current_volunteers_available", ":center_no", slot_center_outlander_volunteers_available),

            (try_begin),
            (lt, ":current_volunteers_available", ":outlanders"),
               (val_add, ":current_volunteers_available", 1),
               (party_set_slot, ":center_no", slot_center_outlander_volunteers_available, ":current_volunteers_available"),
            (else_try),
            (gt, ":current_volunteers_available", ":outlanders"),
               (val_sub, ":current_volunteers_available", 1),
               (party_set_slot, ":center_no", slot_center_outlander_volunteers_available, ":current_volunteers_available"),
            (try_end),

            # Update local volunteers who will take a bribe to join
            (party_get_slot, ":current_volunteers_available", ":center_no", slot_center_bribe_volunteers_available),

            (try_begin),
            (lt, ":current_volunteers_available", ":bribes"),
               (val_add, ":current_volunteers_available", 1),
               (party_set_slot, ":center_no", slot_center_bribe_volunteers_available, ":current_volunteers_available"),
            (else_try),
            (gt, ":current_volunteers_available", ":bribes"),
               (val_sub, ":current_volunteers_available", 1),
               (party_set_slot, ":center_no", slot_center_bribe_volunteers_available, ":current_volunteers_available"),
            (try_end),
  ...
  # Recruitment triggers (channelers) move towards target
  (24*5,
   [
   (try_for_range, ":center_no", centers_begin, centers_end),

        (party_get_slot, ":channelers_available", ":center_no", slot_center_channelers_available),
        (party_get_slot, ":target", ":center_no", slot_center_channelers_available_target),
   
        # Update channelers available

        (try_begin),
        (lt, ":channelers_available", ":target"),
            (val_add, ":channelers_available", 1),
            (party_set_slot, ":center_no", slot_center_channelers_available, ":channelers_available"),
        (else_try),
        (gt, ":channelers_available", ":target"),
            (val_sub, ":channelers_available", 1),
            (party_set_slot, ":center_no", slot_center_channelers_available, ":channelers_available"),
        (try_end),
  ...
  ### Reset the target value for the number of channelers in villages, castles, and towns every 20 days
   ...
   [
       (try_for_range, ":center_no", centers_begin, centers_end),
            ...
            (party_slot_eq, ":center_no", slot_party_type, spt_town),
                ...
                (party_set_slot, ":center_no", slot_center_channelers_available_target, ":random"),
            (else_try),
            (party_slot_eq, ":center_no", slot_party_type, spt_village),
  ...
  # Recruitment triggers (trollocs) move towards target
  (24,
   [
   (try_for_range, ":center_no", centers_begin, centers_end),

        (party_get_slot, ":trollocs_available", ":center_no", slot_center_trollocs_available),
        (party_get_slot, ":target", ":center_no", slot_center_trollocs_available_target),
   
        # Update channelers available

        (try_begin),
        (lt, ":trollocs_available", ":target"),
            (try_begin),
            (eq, ":center_no", "p_town_20"), # Shayol Ghul
                (val_add, ":trollocs_available", 2),
            (else_try),
                (val_add, ":trollocs_available", 1),
            (try_end),
            (party_set_slot, ":center_no", slot_center_trollocs_available, ":trollocs_available"),
        (else_try),
        (gt, ":trollocs_available", ":target"),
            (val_sub, ":trollocs_available", 1),
            (party_set_slot, ":center_no", slot_center_trollocs_available, ":trollocs_available"),
        (try_end),
  ...
  ### Reset the target value for the number of trollocs in villages, castles, and towns every 10 days
   ...
   [
       (try_for_range, ":center_no", towns_begin, towns_end),
           ...
           (party_slot_eq, ":center_no", slot_party_type, spt_town),
                ...
                (party_set_slot, ":center_no", slot_center_trollocs_available_target, ":random"),
           (else_try),
           (party_slot_eq, ":center_no", slot_party_type, spt_village),
       ...
       # make sure there is always a high target for Shayol Ghul
